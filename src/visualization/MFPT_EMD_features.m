...
[~, M] = size(x);
for m=1:M
    F_EMD(:, ss(1) + m) = compute_EMD_feature(x(:, m), m_EMD);
end
...
[~, M] = size(x);
for m=1:M
    F_EMD(:, sum(ss(1:2)) + m) = compute_EMD_feature(x(:, m), m_EMD);
end
...
[~, M] = size(x);
for m=1:M
    F_EMD(:, sum(ss(1:3)) + m) = compute_EMD_feature(x(:, m), m_EMD);
end
...
for f=1:m_EMD
    ...
    plot(F_EMD(f, 1:ss(1)));
    plot(F_EMD(f, (ss(1)+1):sum(ss(1:2))));
    plot(F_EMD(f, (sum(ss(1:2))+1):sum(ss(1:3))));
    plot(F_EMD(f, (sum(ss(1:3))+1):sum(ss)));
