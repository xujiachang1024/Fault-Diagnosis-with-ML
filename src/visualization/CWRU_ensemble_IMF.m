...
for m=1:M
    [~, F_EEMD_a1(:, m)] = generate_ensemble_IMF(x(:, m), M_ens, I_imf);
end
...
for m=1:M
    [~, F_EEMD_a2(:, m)] = generate_ensemble_IMF(x(:, m), M_ens, I_imf);
end
...
for m=1:M
    [~, F_EEMD_a3(:, m)] = generate_ensemble_IMF(x(:, m), M_ens, I_imf);
end
...
for m=1:M
    [~, F_EEMD_a4(:, m)] = generate_ensemble_IMF(x(:, m), M_ens, I_imf);
end
...
for m=1:M
    [~, F_EEMD_a5(:, m)] = generate_ensemble_IMF(x(:, m), M_ens, I_imf);
end
...
for m=1:M
    [~, F_EEMD_a6(:, m)] = generate_ensemble_IMF(x(:, m), M_ens, I_imf);
end
...
for m=1:M
    [~, F_EEMD_a7(:, m)] = generate_ensemble_IMF(x(:, m), M_ens, I_imf);
end
...
for f=1:I_imf
    ...
    title(strcat('CWRU Data: Ensemble IMF #', num2str(f)));
    xlabel('Sample point');
    ylabel(strcat('a', num2str(f)));
    legend('Normal', 'OR 014', 'IR 007', 'IR 014', 'IR 021', 'IR 028', 'B 014', 'Location', 'northeastoutside');
    saveas(fig, strcat('reports/figures/CWRU_ensemble_IMF_a', num2str(f), '.png'));
end